$(function () {
  ...
  let addGallerySubmit = addGalleryForm.find("#addGallerySubmit"); // fetch all images

  fetchAllImages(); // validation on uploaded

  imgFileInput.on("change", function () {
    let fileValue = $(this).val();
    let fileExtension = fileValue.split(".").pop().toLowerCase(); // the allowed files extensions uploaded

    let myExtensions = ["jpg", "jpeg", "png"]; // check on the extension file

    if (myExtensions.includes(fileExtension)) {
      // get the file size with "kb"
      let fileSize = Math.round(this.files[0].size / 1024); // the allowed size

      let mySize = 1024 * 4; // check on the file size
      ...
      if (fileSize <= mySize) {
        ...
        showMsg("success", "the image upload success"); // and make the what you want like preview img ...
      } else {
